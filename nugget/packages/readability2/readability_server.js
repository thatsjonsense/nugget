...
Oauth.registerService('readability', 2, null, function(query) {
...
var getTokenResponse = function (query) {
    ...
    responseContent = Meteor.http.post(
      "https://www.readability.com/api/rest/v1/oauth/access_token/", {
        params: {
          ...
          code: query.code,
          redirect_uri: Meteor.absoluteUrl("_oauth/readability?close")
        }
      }).content;
  } catch (err) {
    throw new Error("Failed to complete OAuth handshake with Readability. " + err.message);
  }

  // If 'responseContent' does not parse as JSON, it is an error.
  if (!isJSON(responseContent)) {
    throw new Error("Failed to complete OAuth handshake with Readability. " + responseContent);
  }
  ...
  if (!accessToken) {
    throw new Error("Failed to complete OAuth handshake with Readabiliy " +
...
var getIdentity = function (accessToken) {
  try {
    return Meteor.http.get('https://www.readability.com/api/rest/v1/users/_current/', {
      params: {oauth2_access_token: accessToken, format: 'json'}}).data;
  } catch (err) {
    throw new Error("Failed to fetch identity from Readability. " + err.message);
  }
};
